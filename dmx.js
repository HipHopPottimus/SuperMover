...
const START_CHANNEL = 1; // DMX channels are 1-indexed

export const CHANNELS = {
    Pan: START_CHANNEL,
    PanFine: START_CHANNEL + 1,
    Tilt: START_CHANNEL + 2,
    TiltFine: START_CHANNEL + 3,
    PTSpeed: START_CHANNEL + 4,
    ColorWheel: START_CHANNEL + 5,
    GoboWheel: START_CHANNEL + 6,
    GoboRotation: START_CHANNEL + 7,
    Prism: START_CHANNEL + 8,
    Focus: START_CHANNEL + 9,
    Dimmer: START_CHANNEL + 10,
    Shutter: START_CHANNEL + 11,
    Function: START_CHANNEL + 12,
    MovementMacros: START_CHANNEL + 13,
    Zoom: START_CHANNEL + 14,
}
...
export const PRISM = {
    ...
    SixFaucetReverseRotation: function (speed) { return 66 + Math.round(58 * Math.max(Math.min(1, speed), 0)); },
    FiveFaucet: 132,
    FiveFaucetRotation: function (speed) { return 135 + Math.round(58 * Math.max(Math.min(1, speed), 0)); },
    FiveFaucetReverseRotation: function (speed) { return 194 + Math.round(58 * Math.max(Math.min(1, speed), 0)); }
}
 ...
 * @param {*} deg The pan angle in degrees (0 to 540)
 * @param {*} useFine Whether to use fine adjustment (16-bit precision). If false, only the coarse value is set (8-bit precision).
 * @returns 
 */
export function setPanDeg(deg, useFine = false) {
    // 0 to 540deg
    const value = Math.round(deg / 540 * 65535);
    if (useFine) {
        const coarse = value >> 8 & 0xFF;
        const fine = value & 0xFF;
        dmxDevice.setChannels({ [CHANNELS.Pan]: coarse, [CHANNELS.PanFine]: fine });
        return;
    }
    dmxDevice.setChannels({ [CHANNELS.Pan]: value >> 8 & 0xFF });
}

/**
 * Sets the tilt angle in degrees.
 * @param {*} deg The tilt angle in degrees (0 to 270)
 * @param {*} useFine Whether to use fine adjustment (16-bit precision). If false, only the coarse value is set (8-bit precision).
 * @returns 
 */
export function setTiltDeg(deg, useFine = false) {
    // 0 to 270deg
    const value = Math.round(deg / 270 * 65535);
    if (useFine) {
        const coarse = value >> 8 & 0xFF;
        const fine = value & 0xFF;
        dmxDevice.setChannels({ [CHANNELS.Tilt]: coarse, [CHANNELS.TiltFine]: fine });
        return;
    }
    dmxDevice.setChannels({ [CHANNELS.Tilt]: value >> 8 & 0xFF });
}

/**
 * Sets the values for multiple DMX channels.
 * @param {*} channels An object mapping channel IDs to their values.
 */
export function setChannels(channels) {
    dmxDevice.setChannels(channels);
}
...
export function init() {
    dmxDevice.setChannels({
        [CHANNELS.Shutter]: SHUTTER.Open,
        [CHANNELS.ColorWheel]: COLORS.White,
        [CHANNELS.GoboWheel]: GOBO.Open,
        [CHANNELS.GoboRotation]: GOBO_ROTATION.NoFunction,
        [CHANNELS.Prism]: PRISM.NoFunction,
        [CHANNELS.Function]: 0,
        [CHANNELS.Zoom]: 0,
        [CHANNELS.Dimmer]: 255
    });
